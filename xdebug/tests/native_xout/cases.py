...
# Every public action has an explicit first-section usability contract.  Tokens
# are intentionally semantic field names rather than fixture-specific values.
FIRST_SECTION_REQUIRED = {
    "actions": ("action_count",),
    "apb.config.list": ("config:", "name"),
    "apb.config.load": ("config:", "name"),
    "apb.export": ("name", "status", "scan_complete", "total_count"),
    "apb.query": ("name", "scan_complete", "total_count"),
    "apb.statistics": ("name", "analysis_complete", "total_count"),
    "apb.transaction.cursor": ("op", "index", "total_count"),
    "apb.transfer_window": ("name", "total_count", "returned_count"),
    "axi.analysis": ("analysis", "min", "p95", "analysis_complete"),
    "axi.channel_stall": ("channel", "stall", "analysis_complete"),
    "axi.config.list": ("config:", "name"),
    "axi.config.load": ("config:", "name"),
    "axi.export": ("status", "output.path", "output.meta_path", "analysis_complete"),
    "axi.latency_outlier": ("candidate_count", "total_count", "returned_count"),
    "axi.outstanding_timeline": ("peak_read", "peak_write", "analysis_complete"),
    "axi.query": ("name", "query_mode", "analysis_complete"),
    "axi.request_response_pair": ("name", "total_count", "analysis_complete"),
    "axi.statistics": ("name", "analysis_complete", "total_count"),
    "axi.transaction.cursor": ("op", "index", "total_count"),
    "batch": ("count", "all_ok", "failed_count"),
    "counter.statistics": ("min_value", "max_value", "analysis_complete"),
    "event.config.list": ("config:", "name", "clock"),
    "event.config.load": ("config:", "name", "clock"),
    "event.export": ("status", "output.path", "analysis_complete"),
    "event.find": ("mode", "first", "last", "analysis_complete"),
    "expr.eval_at": ("expr", "time", "status"),
    "expr.normalize": ("expr", "confidence"),
    "list.add": ("status", "name", "signal"),
    "list.create": ("status", "name", "signal_count"),
    "list.delete": ("name", "deleted"),
    "list.export": ("status", "output.path", "output.manifest_path"),
    "list.first_change": ("name", "changed_signal_count", "diff_found"),
    "list.load": ("loaded", "mode"),
    "list.show": ("name", "signal_count"),
    "list.validate": ("name", "all_found"),
    "nwave.rc.generate": ("written", "output.path", "valid"),
    "protocol.handshake.inspect": ("transfer_count", "analysis_complete"),
    "schema": ("action", "kind", "schema_path"),
    "scope.list": ("path", "kind", "total_count", "returned_count"),
    "scope.roots": ("recommended", "analysis_complete", "total_count"),
    "session.close": ("removed", "session_id"),
    "session.doctor": ("healthy", "session_id"),
    "session.gc": ("removed_count", "kept_count"),
    "session.list": ("session_count",),
    "session.open": ("status", "session_id", "mode", "transport"),
    "signal.anomaly.inspect": ("finding_count", "highest_severity", "analysis_complete"),
    "signal.canonicalize": ("status", "query", "match_count"),
    "signal.changes": ("signal", "actual_transition_count", "analysis_complete"),
    "signal.resolve": ("query", "total_count", "returned_count"),
    "signal.sampled_pulse.inspect": ("unsampled_valid_pulse_count", "analysis_complete"),
    "signal.stability": ("stable", "analysis_complete", "total_count"),
    "signal.statistics": ("signal", "sample_count", "analysis_complete"),
    "signal.xz_verify": ("signal", "expected_state", "verdict"),
    "stream.config.get": ("stream:", "name", "clock"),
    "stream.config.list": ("count",),
    "stream.config.load": ("loaded", "mode"),
    "stream.describe": ("config:", "name", "clock"),
    "stream.export": ("status", "output.path", "output.meta_path", "analysis_complete"),
    "stream.query": ("query", "found", "packet_index", "analysis_complete"),
    "stream.validate": ("stream", "ok", "analysis_complete"),
    "trace.active_driver": ("signal", "time", "termination", "analysis_complete"),
    "trace.active_driver_chain": ("signal", "time", "termination", "analysis_complete"),
    "trace.driver": ("signal", "analysis_complete", "truncation_scopes"),
    "trace.load": ("signal", "analysis_complete", "truncation_scopes"),
    "trace.x_origin": ("signal", "time", "termination", "analysis_complete"),
    "value.at": ("values:", "name"),
    "verify.conditions": ("verdict", "condition_count", "failed"),
    "waveform.cursor.delete": ("name", "status"),
    "waveform.cursor.get": ("name", "time", "status"),
    "waveform.cursor.list": ("cursor_count", "active_cursor"),
    "waveform.cursor.set": ("name", "time", "status"),
    "waveform.cursor.use": ("active_cursor", "time"),
    "window.verify": ("verdict", "proof_begin", "proof_end", "analysis_complete"),
}

SUMMARY_OMITTED_ACTIONS = frozenset({
    "apb.config.list", "apb.config.load", "axi.config.list", "axi.config.load",
    "event.config.list", "event.config.load", "stream.config.get",
    "stream.describe",
})
