...
bool parse_group(const Json& item, RcGroup& group, std::string& err) {
    ...
    if (item.contains("expanded")) {
        err = "group.expanded is not supported; nwave.rc.generate emits addGroup/addSubGroup without -e";
        return false;
    }
...
bool parse_rc_config_json(const Json& doc, RcConfig& cfg, std::string& err) {
    ...
    if (doc.contains("window_time_unit")) {
        err = "window_time_unit is not supported; nwave.rc.generate always emits windowTimeUnit 1ns";
        return false;
    }
...
std::string render_signal_rc(const RcConfig& cfg) {
    std::ostringstream os;
    os << "; Generated by xdebug nwave.rc.generate\n";
