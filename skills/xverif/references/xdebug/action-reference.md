...
| `scope.list` | stable | waveform | 分类列出 FSDB module、port 和 signal。 | `args.level=0` 只返回当前 module 的声明 port/signal、interface 实例（归入 signal）、interface 端口（归入 port，`direction=interface`）和直接子 module；1/2/3 继续进入相应 module 层级。`include_patterns`/`exclude_patterns` 用区分大小写的 `*`/`?` glob 匹配相对 `summary.path` 的 name，exclude 优先且不剪枝；`kind` 接受 all、module、port、signal。结果不进入 interface，也不展开 composite 成员。 | 用相对 name 确认真实层次、端口、信号和 interface 声明。 | optional: path, level(default 0), kind(default all), include_patterns, exclude_patterns<br>limits: max_rows |
