...
- `value.at` / `value.batch_at`：`clock` 可选；省略时直接读取 `time` 的 FSDB 最终值并返回 `sampling_mode:"raw_time"`，传入时保留 clock-sampled 上下文。无 `clock` 时不得传 `edge` / `sample_point`。用 `args.value_format:"hex"|"bin"|"dec"` 控制响应显示，默认 hex；取得 NPI range size 后，值以 `<实际位宽>'h` / `<实际位宽>'b` / `<实际位宽>'d` 返回。decimal 遇 X/Z 会明确报告 binary effective format。`args.format` 不再是兼容别名；aggregate 应传入最终 leaf signal，xdebug 不猜测 indexed path。
...
| `trace.x` | experimental | combined | 从指定 signal/time 追溯 X。 | 先确认查询值含 X，再按 DFS 同等追踪含 X 的 RHS/control；`query_time` 保留请求时刻，逐跳 `x_onset_time` 表示连续 X 区间起点，`active_time` 表示 NPI active-driver 时刻。深度停止时以 `continue_time` 返回可直接续查的 frontier。 | 找到一个或多个 X 候选源、不可见边界或受限现场。 | required: signal, time<br>optional: value_format<br>limits: max_depth, max_nodes, max_time_steps, max_trace_signals, max_chains(default 8) |
