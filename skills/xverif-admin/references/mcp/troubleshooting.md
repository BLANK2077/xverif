...
1. 工具不可见：调用 `xverif_tools`，先检查工具组开关；生命周期工具再检查 `XVERIF_MCP_ENABLE_MUTATION`，batch 再检查 `XVERIF_MCP_ENABLE_ARTIFACT_WRITE`。
...
- `MCP_MUTATION_DISABLED`：请求会改变 session/backend 状态；按需设置 `XVERIF_MCP_ENABLE_MUTATION=1` 后重启 server。
